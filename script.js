...
async function searchUser() {
	...
	try {
		...
		const user = users.find(
			(u) => `${u.first_name.toLowerCase()} ${u.last_name.toLowerCase()}` === searchInput.toLowerCase()
		);
		...
		if (user) {
			resultDiv.innerHTML = `
				<div class="max-w-sm rounded overflow-hidden shadow-lg bg-white">
					<img class="w-full" src="${user.avatar}" alt="${user.first_name} ${user.last_name}">
					<div class="px-6 py-4">
						<div class="font-bold text-xl mb-2">${user.first_name} ${user.last_name}</div>
						<p class="text-gray-700 text-base">Email: ${user.email}</p>
						<p class="text-gray-700 text-base">IP Address: ${user.ip_address}</p>
					</div>
                    </div>
                    `;
			timeTakenDiv.innerHTML = `経過時間: ${timeTaken.toFixed(4)} 秒`;
		} else {
			resultDiv.innerHTML = "ユーザーが見つかりませんでした";
		}
	} catch (error) {
